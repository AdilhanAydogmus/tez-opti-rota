...
class VRPData:
    def __init__(self, filepath, segment_filepath):
        ...
        try:
            ...
            # Sheet1 bulunamazsa ilk sayfayı al
            try:
                df_seg = pd.read_excel(segment_filepath)
            except Exception:
                # Segment dosyası okunamazsa boş DataFrame ile devam et
                df_seg = pd.DataFrame(columns=["Store", "Segment"])
